...
<div id="kohana-debug-toolbar">
	...
	<div id="debug-toolbar" class="debug-toolbar-align-<?php echo $align ?>">
		...
		<!-- Kohana icon -->
		<?php if (Kohana::$config->load('debug_toolbar.toolbar.minimized') === TRUE): ?>
		...
		<?php endif ?>
			...
			<?php if (Kohana::$config->load('debug_toolbar.panels.benchmarks')): ?>
				...
				<li id="time" onclick="debugToolbar.show('debug-benchmarks'); return false;">
					...
					<?php echo round(($data['benchmarks']['application']['total_time'])*1000, 2) ?> ms
				</li>
				<!-- Memory -->
				<li id="memory" onclick="debugToolbar.show('debug-benchmarks'); return false;">
					...
					<?php echo Text::bytes($data['benchmarks']['application']['total_memory']) ?>
				</li>
			<?php endif ?>
			...
			<?php if (Kohana::$config->load('debug_toolbar.panels.database')): ?>
				<li id="toggle-database" onclick="debugToolbar.show('debug-database'); return false;">
					...
					<?php echo isset($data['queries']) ? $data['queries']['count'] : 0 ?>
				</li>
			<?php endif ?>
	...
	<?php if (Kohana::$config->load('debug_toolbar.panels.benchmarks')): ?>
		<div id="debug-benchmarks" class="top" style="display: none;">
			...
			<table cellspacing="0" cellpadding="0">
				...
				<?php if (count($data['benchmarks'])):
					$application = array_pop($data['benchmarks']);?>
					<?php foreach ((array)$data['benchmarks'] as $group => $marks): ?>
						<tr>
	...
	<?php if (Kohana::$config->load('debug_toolbar.panels.database')): ?>
		<div id="debug-database" class="top" style="display: none;">
			...
			<table cellspacing="0" cellpadding="0">
				...
				<?php foreach ($data['queries']['data'] as $db_profile => $stats):
				...
				<?php endforeach; ?>
				<?php if (count($data['queries']['data']) > 1): ?>
					<tr>
						<th colspan="2" align="left"><?php echo $data['queries']['count'] ?> TOTAL</th>
						<th><?php echo number_format($data['queries']['time'] * 1000, 3) ?> ms</th>
						<th><?php echo number_format($data['queries']['memory'] / 1024, 3) ?> kb</th>
					</tr>
				<?php endif; ?>
			</table>
		</div>
	...
	<?php if (Kohana::$config->load('debug_toolbar.panels.files')): ?>
		<div id="debug-files" class="top" style="display: none;">
			...
			<table cellspacing="0" cellpadding="0">
				...
				<?php foreach ((array)$data['files']['list'] as $id => $file_data): ?>
					<tr class="<?php echo text::alternate('odd','even')?>">
						<td><?php echo $id + 1 ?></td>
						<td><?php echo $file_data['name'] ?></td>
						<td><?php echo $file_data['size'] ?></td>
						<td><?php echo $file_data['lines'] ?></td>
					</tr>
				<?php endforeach; ?>
				<tr align="left">
					<th colspan="2">total</th>
					<th><?php echo text::bytes($data['files']['total_size']) ?></th>
					<th><?php echo number_format($data['files']['total_lines']) ?></th>
				</tr>
			</table>
		</div>
	<?php endif ?>
	...
	<?php if (Kohana::$config->load('debug_toolbar.panels.modules')):
		...
		<div id="debug-modules" class="top" style="display: none;">
			...
			<table cellspacing="0" cellpadding="0">
				...
				<?php foreach($data['modules']['list'] as $id => $module_data): ?>
				<tr class="<?php echo text::alternate('odd','even')?>">
					<td><?php echo $id + 1 ?></td>
					<td><?php echo $module_data['name'] ?></td>
					<td><?php echo $module_data['path'] ?></td>
					<td><?php echo $module_data['abs_path'] ?></td>
				</tr>
				<?php endforeach ?>
			</table>
		</div>
	...
	<?php if (Kohana::$config->load('debug_toolbar.panels.routes')):
		...
		<div id="debug-routes" class="top" style="display: none;">
			...
			<table cellspacing="0" cellpadding="0">
				...
				<?php foreach($data['routes']['list'] as $id => $route_data):
						$class = ($route_data['current'] ? ' current' : ''); ?>
				<tr class="<?php echo text::alternate('odd','even').$class?>">
					<td><?php echo $id + 1 ?></td>
					<td><?php echo $route_data['name'] ?></td>
				</tr>
				<?php endforeach ?>
			</table>
		</div>
	...
	<!-- Custom data-->
	<?php if (Kohana::$config->load('debug_toolbar.panels.customs') && count($data['customs']) > 0):
			$r_counter = 0; ?>
		<div id="debug-customs" class="top" style="display: none;">
			<h1>Custom data</h1>
			<ul class="sectionmenu">
				<?php foreach($data['customs'] as $section => $custom_data): ?>
